...
The `--debug` flag is available on every command that calls the LLM:

```bash
spectral mcp analyze myapp --debug
spectral openapi analyze myapp -o myapp-api --debug
spectral graphql analyze myapp -o myapp-api --debug
spectral auth analyze myapp --debug
spectral auth extract myapp --debug
spectral auth login myapp --debug
```
