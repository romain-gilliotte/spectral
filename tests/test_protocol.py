...
class TestDetectTraceProtocol:
    ...
    def test_graphql_mutation_by_body(self):
        body = json.dumps({"query": "mutation { createUser(name: \"test\") { id } }"}).encode()
        trace = make_trace(
            "t_0001", "POST", "https://api.example.com/api", 200, 1000,
            request_body=body,
            request_headers=[Header(name="Content-Type", value="application/json")],
    ...
    def test_grpc_by_content_type(self):
        trace = make_trace(
            "t_0001", "POST", "https://api.example.com/service", 200, 1000,
    ...
    def test_binary_by_content_type(self):
        trace = make_trace(
            "t_0001", "POST", "https://api.example.com/data", 200, 1000,
            request_headers=[Header(name="Content-Type", value="application/octet-stream")],
        )
    ...
    def test_protobuf_detected_as_binary(self):
        trace = make_trace(
            "t_0001", "POST", "https://api.example.com/data", 200, 1000,
            request_headers=[Header(name="Content-Type", value="application/x-protobuf")],
        )
        assert detect_trace_protocol(trace) == "binary"
...
class TestDetectWsProtocol:
    def test_graphql_ws_by_protocol(self):
        ws = make_ws_connection("ws_0001", "wss://example.com/ws", 1000, protocols=["graphql-ws"])
        assert detect_ws_protocol(ws) == "graphql-ws"

    def test_graphql_ws_by_message(self):
        msg = make_ws_message(
            "ws_0001_m001", "ws_0001", 1001, "send",
    ...
    def test_graphql_subscribe_message(self):
        msg = make_ws_message(
            "ws_0001_m001", "ws_0001", 1001, "send",
            json.dumps({"type": "subscribe", "id": "1", "payload": {"query": "subscription { data }"}}).encode(),
        )
        ws = make_ws_connection("ws_0001", "wss://example.com/ws", 1000, messages=[msg])
        assert detect_ws_protocol(ws) == "graphql-ws"
