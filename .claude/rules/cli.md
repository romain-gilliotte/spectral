...
| `TokenState` | `formats/mcp_tool.py` | Pydantic model: persisted auth state (headers, body_params, refresh_token, expires) |
...
├── token.json            # TokenState: current auth headers, body_params, refresh_token, expires_at
├── tools/
...
- **`auth set`** calls `write_token` to persist manually-provided headers/cookies/body params.
