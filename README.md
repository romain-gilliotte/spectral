...
Reverse-engineer any app's private API. Browse normally, get a full OpenAPI spec — then use it to build AI agents and automations instead of brittle browser scripts.

Most apps sit on undocumented APIs that work perfectly well. But without a spec, people fall back to Playwright/Selenium/Puppeteer: slow, fragile, breaks on every UI change, can't handle mobile. Spectral captures the traffic, has an LLM figure out what each call means, and gives you an OpenAPI 3.1 spec you can actually use.

## How it works

1. **Capture** — Chrome extension (web) or MITM proxy (Android) records traffic + UI actions while you browse
2. **Analyze** — LLM correlates UI actions with API calls, infers endpoint patterns, auth flow, and business meaning — outputs an OpenAPI 3.1 spec
...
uv run spectral analyze capture.zip -o spec.yaml
```
...
```
spectral analyze <bundle> -o spec.yaml [--model MODEL] [--debug] [--skip-enrich]
```
